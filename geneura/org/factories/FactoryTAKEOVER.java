...
public class FactoryTAKEOVER {
	
	public static Algorithm[] createBinaryNewscastTakeOver(){
		...
		TAKEOVER evag[] = new TAKEOVER[Configuration.population_size];
		for (int i=0;i<Configuration.population_size;i++){
			evag [i]=(TAKEOVER)createBinaryTakeOver(observer,evaluator);
			evag[i].set_index(i);
		...
		evag[0].getEvaluator().evaluate(evag[0].this_individual());
	...
	public static Algorithm createBinaryWatssStrogatzTakeOver(){
		ClassLoader loader = ClassLoader.getSystemClassLoader();
		Observer observer = new Observer(Configuration.statistic_sample);
		CommonState.setSeed(Configuration.seed);
		Individual._min = Configuration.minimization;
	...
	public static Algorithm createBinaryPanTakeOver(){
		ClassLoader loader = ClassLoader.getSystemClassLoader();
		Observer observer = new Observer(Configuration.statistic_sample);
		CommonState.setSeed(Configuration.seed);
		Individual._min = Configuration.minimization;
	...
	private static Algorithm createBinaryTakeOver(Observer observer,Evaluator evaluator){
		...
		TAKEOVER evag = new TAKEOVER(individual);
