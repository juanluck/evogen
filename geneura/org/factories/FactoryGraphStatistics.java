...
import geneura.org.graphs.StandAlonePanmictic;
...
import geneura.org.takeover.TAKEOVER;
...
public class FactoryGraphStatistics {

	public static Algorithm createNewscastGraphStatistics(){
		...
		//One of the individuals is optimum for Traps
		for(int i=0;i<Configuration.chromosome_size;i++)
			evag[0].this_individual().getChr().setGen(i, new Boolean(true));
		
		evaluator.evaluate(evag[0].this_individual());
		...
		StandAloneNewscast sanws = new StandAloneNewscast();
		sanws.setAlgorithm(sn, evaluator, null, selection, crossover, mutation, termination);
	
		sanws.attach(observer);
		Observer.restart();
		return sanws;
		
	}
	
public static Algorithm createPanmicticGraphStatistics(){
		...
		//Creating the topology
		PanmicticGraph pg = new PanmicticGraph(evag);
		
		
		observer.registerObservable(pg);
		...
		if (Configuration.termination_hit_value){
			ValueReached aux; 
			if (Configuration.termination_value_is_reached == Double.MIN_VALUE)
				aux = new ValueReached(evaluator.hit_value(),pg);
			else
				aux = new ValueReached(Configuration.termination_value_is_reached,pg);
		...
		if (Configuration.termination_not_improved){
			SolutionNotImproved aux = new SolutionNotImproved(pg);
			termination_criteria[index_criteria] = (ITermination)aux;
			observer.registerObservable((IObservable)aux);
			index_criteria++;
		}
		...
		StandAlonePanmictic sapan = new StandAlonePanmictic();
		sapan.setAlgorithm(pg, evaluator, null, selection, crossover, mutation, termination);
	
		sapan.attach(observer);
		Observer.restart();
		return sapan;
