...
import geneura.org.evoag.EVAGseqGPChurn;
...
public class FactoryEVAGseqGP {

		public static Algorithm createNewscastEVAGseqGP(){
			...
			EVAGseqGP evagseq = new EVAGseqGP();
			if (selections==null)
				evagseq.setAlgorithm(sn, evaluator, null, selection, crossover, mutation, termination);
			else
				evagseq.setAlgorithmS(sn, evaluator, null, selections, crossover, mutation, termination);
		...
		public static Algorithm createNewscastEVAGseqGPChurn(){
			...
			EVAGChurn evag[] = new EVAGChurn[Configuration.population_size];

			for (int i=0;i<Configuration.population_size;i++){
				GPIndividual individual = new SimpleGPIndividual(evaluator.getPrimitives(), evaluator, problemData, i);
				evag[i] = new EVAGChurn(individual,traces);
				evag[i].set_index(i);
				evaluator.evaluate(evag[i].this_individual());
			}
			...
			//Creating the topology
			SimpleNewscastChurn sn = new SimpleNewscastChurn(evag);
			...
			EVAGseqGPChurn evagseq = new EVAGseqGPChurn();
			if (selections==null)
				evagseq.setAlgorithm(sn, evaluator, null, selection, crossover, mutation, termination);
			else
				evagseq.setAlgorithmS(sn, evaluator, null, selections, crossover, mutation, termination);
		...
		public static Algorithm createNewscastEVAGseqGPADF(){
			...
			IGenomeValidator genomeV = new ADFGenomeValidator(Configuration.gp_tree_depth, evaluator);
