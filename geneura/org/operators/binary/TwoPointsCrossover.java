...
public class TwoPointsCrossover extends Crossover{
	...
	public TwoPointsCrossover(double pc) {
		super(pc);
	}
	public Individual cross(Individual father, Individual mother) {
		if (CommonState.r.nextDouble()<_pc){
			...
			while (pos1==pos2){// || pos1+1==pos2 || pos2+1==pos1){
				pos1 = CommonState.r.nextInt(father.getChr().getLength());
				pos2 = CommonState.r.nextInt(father.getChr().getLength());

			}

			Individual ind = (Individual) mother.clone();
			
			for (int i = pos1;i!=pos2;i=(i+1)%father.getChr().getLength()){
